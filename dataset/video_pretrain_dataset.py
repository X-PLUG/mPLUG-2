...
from .video_utils.utils import read_frames_decord, read_from_tar, read_frames_gif
...
class pretrain_dataset_video(Dataset):
    def __init__(self, ann_file, transform, video_path, num_frames=8, max_words=30, read_local_data=True):
        ...
        if not self.read_local_data:
            bucket_name = "xxxx" if "2m" in ann_file[0] else "xxxxx"
            ...
            for i in range(3):
                if self.read_local_data:
                    ...
                    try:
                        ...
                        if '.gif' in video_buffer:
                            video_array = read_frames_gif(video_buffer, num_frames=self.num_frames, sample='rand')
                        else:
