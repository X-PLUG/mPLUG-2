...
class coco_caption(Dataset):
    def __init__(self, ann_file, transform, max_words=30, num_frames=16):
    ...
    def __getitem__(self, index):
        ...
        while True:
            if not os.path.exists(video_path):
                video_path = ann['image_path']
            try:
                video_array = Image.open(video_path).convert('RGB')
            except:
                time.sleep(0.01)
                index = 0 if index == (len(self) - 1) else index + 1
                ann = self.ann[index]
        ...
        video_array = video_array.unsqueeze(1).repeat(1, self.num_frames, 1, 1)
...
class coco_dataset_scst(Dataset):
    def __init__(self, ann_file, transform, root_path, max_words=30, read_local_data=True, is_train=True, add_object=False):
        self.ann = []
        for f in ann_file:
            self.ann += json.load(open(f,'r'))
    ...
    def __getitem__(self, index):    
        ...
        if self.read_local_data:
            ...
            while not self.bucket.object_exists("xxxxx/"+ann['image']):
                index = 0 if index == (len(self) - 1) else index + 1
                ann = self.ann[index]
            while True:
                try:
...
class pretrain_dataset_4m(Dataset):
    def __init__(self, ann_file, transform, max_words=30, read_local_data=True, image_root="", epoch=None):
        ...
        if not self.read_local_data:
            ...
            auth = oss2.Auth("xxxxxxxxxx", "xxxxxxxx")
            self.bucket = oss2.Bucket(auth, "xxxxxxxx", bucket_name) 
