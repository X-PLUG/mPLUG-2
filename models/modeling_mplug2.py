...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        self.has_cross_attention = getattr(self.config, "add_cross_attention", False)
        if self.has_cross_attention or (layer_num >= (config.num_hidden_layers-config.num_uni_layer)):           
            self.crossattention = BertAttention(config, is_cross_attention=True)
        if layer_num == config.num_hidden_layers-config.num_uni_layer:
            self.latents = nn.Parameter(torch.randn(50, config.hidden_size))
            # self.latents = nn.Parameter(torch.randn(10, config.hidden_size))
            self.has_latent = True
        else:
            self.has_latent = False
            # self.media_pos_emb = nn.Parameter(torch.randn(50, 1, config.hidden_size))
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
        if config.num_hidden_layers-config.num_uni_layer <= layer_num < config.num_hidden_layers:
            self.intermediate_vision = BertIntermediate(config)
            self.output_vision = BertOutput(config)
    ...
    def forward(
        ...
        if mode == "v_module":
            if self.has_latent:
                hidden_states = repeat(self.latents, 'n d -> b n d', b=hidden_states.shape[0])
                attention_mask = None
            else:
                attention_mask = None
            if self.layer_num == (self.config.num_hidden_layers-1):
                encoder_hidden_states, hidden_states = hidden_states, encoder_hidden_states
                encoder_attention_mask = None
                attention_mask = None
        ...
        if self.has_cross_attention or mode == "v_module":
            assert encoder_hidden_states is not None, "encoder_hidden_states must be given for cross-attention layers"
        ...
        # if mode == "v_module":
        #     layer_output = apply_chunking_to_forward(
        #         self.feed_forward_chunk_vision, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        #     )
        # else:
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
    ...
    def feed_forward_chunk_vision(self, attention_output):
        intermediate_output = self.intermediate_vision(attention_output)
        layer_output = self.output_vision(intermediate_output, attention_output)
        return layer_output
        ...
        for i in range(self.start_layer, len(self.layer)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    encoder_hidden_states,
                    encoder_attention_mask,
                    torch.tensor([i-self.start_layer]),
                )
            else:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.uni_layer = config.num_uni_layer
    ...
    def forward(
        ...
        if mode == "text":
            start_layer = 0
            output_layer = len(self.layer)-self.uni_layer
        elif mode == "v_module":
            start_layer = len(self.layer)-self.uni_layer
            output_layer = len(self.layer)
        else:
            start_layer = 0
            output_layer = len(self.layer)
        ...
        for i in range(start_layer, output_layer):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                def create_custom_forward(module, mode):
                    def custom_forward(*inputs):
                        return tuple(module(*inputs, past_key_value, output_attentions, mode))
...
class SimpleTransformer(BertPreTrainedModel):
    """
    基于BertModel简化的Transformer 只保留了BertEncoder
    """
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        input_shape=encoder_embeds.shape
