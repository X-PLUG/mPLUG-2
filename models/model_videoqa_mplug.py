...
from models.modeling_mplug2 import BertConfig, BertModel, BertLMHeadModel, FusionModel
...
class MPLUG2(nn.Module):
    ...
    def forward(self, image, question, answer=None, alpha=0, k=None, weights=None, train=True, rerank=False):
        ...
        if train:               
            ...
            if self.distill:                    
                with torch.no_grad():
                    ...
                    image_output_m = self.text_encoder_m(encoder_embeds=image_embeds_m,
                                                      attention_mask=image_atts,
                                                      encoder_hidden_states=image_embeds_m,
                                                      encoder_attention_mask=image_atts,
                                                      return_dict=True,
                                                      mode='v_module')
                    image_embeds_m = image_embeds_m + image_output_m.last_hidden_state
                    image_embeds_m = self.visn_emb_dropout_m(self.visn_emb_norm_m(image_embeds_m))
                    image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
            ...
            merge_text_attention = torch.cat([image_atts, question.attention_mask], 1)
            if rerank:
                topk_ids, topk_probs = self.rank_answer(question_output, merge_text_attention, 
    ...
    def module_setting(self, config):
        ...
        self.config_decoder.gradient_checkpointing = True
    ...
    def init_distill(self, config):
        ...
        if self.distill:
            self.visual_encoder_m, _ = initialize_clip_video(config, T=config['model_num_frames'], use_checkpoint=config.get('use_checkpoint', True))
            ...
            self.visn_emb_norm_m = nn.LayerNorm(self.text_width, eps=1e-12)
            self.visn_emb_dropout_m = nn.Dropout(self.config_encoder.hidden_dropout_prob)
            self.model_pairs.append([self.visn_emb_norm, self.visn_emb_norm_m])
            if self.config_encoder.hidden_size != config['vision_width']:
